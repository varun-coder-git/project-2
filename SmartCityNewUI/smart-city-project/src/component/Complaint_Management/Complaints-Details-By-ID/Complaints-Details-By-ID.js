...
class ComplaintsDetailsByID extends Component {
  ...
  getComplaintDetails() {
    ...
    fetch(
      process.env.REACT_APP_API_URL+"Complaint/GetComplaintSummaryById",
      requestOptions
    )
  ...
  sendComment() {
    if (this.state.description != "") {
      ...
      fetch(
        process.env.REACT_APP_API_URL+"Complaint/AddComment",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  DeleteACard = () => {
    ...
    fetch(
      process.env.REACT_APP_API_URL+"Complaint/DeleteComment",
      requestOptions
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  submitFeedBack_SAT() {
    ...
    if (this.state.rating || this.state.feedBack) {
      ...
      fetch(
        process.env.REACT_APP_API_URL+"Complaint/GetComplaintFeedback",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
...
downloadAttachmentFile =(AttachmentDetails) => {
        var file_path=process.env.REACT_APP_API_URL + AttachmentDetails.complaint_media_file;
        var splited_arrey=file_path.substr(file_path.lastIndexOf("/") + 1).split('.');
        var extension=splited_arrey[splited_arrey.length-1]
  ...
  ReOpenComplaint = (CardID) => {
    ...
    fetch(
      process.env.REACT_APP_API_URL+"Complaint/UpdateStatus",
      requestOptions
    )
      .then((response) => response.json())
      .then((data) => {
        ...
        else if(this.state.APIResponse.message === "7 days limit exceeded, you can't reopen now"){
          toast.warn("7 days limit exceeded, you can't reopen now", {
            position: "top-center",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
        ...
        <Container fluid className="header-style">    
          <Row className="header-height-style_SAT">
            <Col xs="2" sm="2" md="2" lg="2" xl="2" xxl="2">
              <Link to={sessionStorage.getItem("complaintDetailsIdBack")}>
                <Tooltip
                  title="Dashboard"
                  placement="top"
                    ...
                    <Row>
                      ...
                      >
                        ...
                        <Row className="card-image-imfo-row-style">
                          <label>
                            {this.state.SearchData.ComplaintPostedDate}
                          </label>
                        </Row>
                      </Col>
                    </Row>
