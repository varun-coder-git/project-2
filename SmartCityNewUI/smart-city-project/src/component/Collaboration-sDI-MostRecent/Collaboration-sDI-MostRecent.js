...
class CollaborationSDIMostRecent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      APIResponse: "",
      cardCounts: sessionStorage.getItem("MrIdeaCardCount")?parseInt(sessionStorage.getItem("MrIdeaCardCount")):4,
      DeleteSubmit: false,
  ...
  componentDidMount() {
    this.getAllIdeas();
    if(sessionStorage.getItem("MrIdeaCardCount"))
    {
  ...
  getAllIdeas() {
    ...
    fetch(process.env.REACT_APP_API_URL+"Ideas/GetIdeas", requestOptions)
  ...
  CardDetails = (CardID) => {
    if (
      ...
      sessionStorage.setItem("MrIdeaCardCount",this.state.cardCounts);
