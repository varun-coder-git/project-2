...
class CollaborationPSMostRecent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      APIResponse: "",
      cardCounts:sessionStorage.getItem("MrPollsCardCount")?parseInt(sessionStorage.getItem("MrPollsCardCount")):4,
  ...
  componentDidMount() {
    this.getAllPolls();
    if(sessionStorage.getItem("MrPollsCardCount"))
    {
  ...
  getAllPolls() {
    ...
    fetch(process.env.REACT_APP_API_URL+"Poll/GetRecentPoll", requestOptions)
  ...
  CardDetails = (CardValues) => {
    if (
      ...
      sessionStorage.setItem("MrPollsCardCount",this.state.cardCounts);
      ...
      <Container className="tab-card-container-style">
        ...
        <div ref={this.Refforcard}></div>
          {this.state.pollsBinding.map((CardValues, index) => (
            <Col xs="12" sm="12" md="6" lg="6" xl="6" xxl="6" className="" ref={(sessionStorage.getItem("poll_id")==CardValues.poll_id) ? this.Refforcard:""}>
