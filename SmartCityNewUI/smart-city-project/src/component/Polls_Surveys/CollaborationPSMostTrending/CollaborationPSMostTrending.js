...
class CollaborationPSMostTrending extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      APIResponse: "",
      cardCounts:sessionStorage.getItem("MtPollsCardCount")?parseInt(sessionStorage.getItem("MtPollsCardCount")):4,
  ...
  componentDidMount() {
    ...
    if(sessionStorage.getItem("MtPollsCardCount"))
    {
  ...
  getMostTrendingPolls() {
    ...
    fetch(process.env.REACT_APP_API_URL+"Poll/MostTrending", requestOptions)
  ...
  CardDetails = (CardValues) => {
    if (
      ...
      sessionStorage.setItem("MtPollsCardCount",this.state.cardCounts);
